...
def rnn_step_backward(dnext_h, cache):
  ...
  tanh_out, x, prev_h, Wx, Wh = cache

  # Use that d/dt(tanh) = 1-tanh^2
  dtanh = (1-tanh_out*tanh_out)*dnext_h
  db = np.sum(dtanh,0)
...
def rnn_backward(dh, cache):
  ...
  dx = np.zeros([N,T,dx1.shape[1]])
  dx[:,T-1,:] = dx1

  for t in reversed(range(0,T-1)):
    dx[:, t, :], dprev_h, dWx_new, dWh_new, db_new = rnn_step_backward(dh[:,t,:]+dprev_h, cache[t])
    dWx = dWx + dWx_new
    dWh = dWh + dWh_new
    db = db + db_new
  dh0 = dprev_h
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
  ...
  i = sigmoid(a[:, range(0, H)])
  f = sigmoid(a[:, range(H, 2*H)])
  o = sigmoid(a[:, range(2*H, 3*H)])
  g = np.tanh(a[:, range(3*H, 4*H)])
  next_c = f*prev_c + i*g
  next_h = o*np.tanh(next_c)

  cache = (x, Wx, Wh, i, f, o, g, x, prev_h, prev_c, np.tanh(next_c))
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
  ...
  #############################################################################
  x, Wx, Wh, i, f, o, g, x, prev_h, prev_c, tanh_c = cache
...
def lstm_backward(dh, cache):
  ...
  N, T, H = np.shape(dh)
  dc = np.zeros([N,H])
  dx1, dprev_h, dc, dWx, dWh, db = lstm_step_backward(dh[:, T-1, :], dc,  cache[T-1])
  dx = np.zeros([N,T,dx1.shape[1]])
  dx[:,T-1,:] = dx1

  for t in reversed(range(0,T-1)):
    dx[:, t, :], dprev_h, dc, dWx_new, dWh_new, db_new = lstm_step_backward(dh[:,t,:]+dprev_h, dc, cache[t])
    dWx = dWx + dWx_new
    dWh = dWh + dWh_new
    db = db + db_new
  dh0 = dprev_h
